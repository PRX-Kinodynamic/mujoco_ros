...
ProductLieGroupV43<G, H> ProductLieGroupV43<G, H>::operator*(const ProductLieGroupV43& other) const
{
  checkMatchingDimensions(other, "operator*");
  return ProductLieGroupV43(traits<G>::Compose(this->first, other.first),
                            traits<H>::Compose(this->second, other.second));
}

template <typename G, typename H>
ProductLieGroupV43<G, H> ProductLieGroupV43<G, H>::inverse() const
{
  return ProductLieGroupV43(traits<G>::Inverse(this->first), traits<H>::Inverse(this->second));
}

template <typename G, typename H>
ProductLieGroupV43<G, H> ProductLieGroupV43<G, H>::retract(const TangentVector& v, ChartJacobian H1,
                                                           ChartJacobian H2) const
{
  const size_t firstDimension = firstDim();
  const size_t secondDimension = secondDim();
  const size_t productDimension = combinedDimension(firstDimension, secondDimension);
  if (static_cast<size_t>(v.size()) != productDimension)
  {
    throw std::invalid_argument(
        "ProductLieGroupV43::retract tangent dimension does not match product "
        "dimension");
  }
  Jacobian1 D_g_first;
  ...
  G g = traits<G>::Retract(this->first, tangentSegment<G>(v, 0, firstDimension), H1 ? &D_g_first : nullptr,
                           H2 ? &D_g_second : nullptr);
  H h = traits<H>::Retract(this->second, tangentSegment<H>(v, firstDimension, secondDimension),
  ...
  if (H1)
  {
    *H1 = zeroJacobian(productDimension);
    ...
    H1->block(firstDimension, firstDimension, secondDimension, secondDimension) = D_h_first;
  }
  if (H2)
  {
    *H2 = zeroJacobian(productDimension);
    H2->block(0, 0, firstDimension, firstDimension) = D_g_second;
    H2->block(firstDimension, firstDimension, secondDimension, secondDimension) = D_h_second;
  }
  return ProductLieGroupV43(g, h);
}

template <typename G, typename H>
typename ProductLieGroupV43<G, H>::TangentVector ProductLieGroupV43<G, H>::localCoordinates(const ProductLieGroupV43& g,
                                                                                            ChartJacobian H1,
                                                                                            ChartJacobian H2) const
{
  ...
  typename traits<G>::TangentVector v1 =
      traits<G>::Local(this->first, g.first, H1 ? &D_g_first : nullptr, H2 ? &D_g_second : nullptr);
  typename traits<H>::TangentVector v2 =
      traits<H>::Local(this->second, g.second, H1 ? &D_h_first : nullptr, H2 ? &D_h_second : nullptr);
  if (H1)
  {
    *H1 = zeroJacobian(productDimension);
    H1->block(0, 0, firstDimension, firstDimension) = D_g_first;
...
template <typename G, typename H>
ProductLieGroupV43<G, H> ProductLieGroupV43<G, H>::compose(const ProductLieGroupV43& other, ChartJacobian H1,
                                                           ChartJacobian H2) const
{
  ...
  H h = traits<H>::Compose(this->second, other.second, H1 ? &D_h_second : nullptr);
  if (H1)
  {
    *H1 = zeroJacobian(productDimension);
    H1->block(0, 0, firstDimension, firstDimension) = D_g_first;
    H1->block(firstDimension, firstDimension, secondDimension, secondDimension) = D_h_second;
  }
  if (H2)
    *H2 = identityJacobian(productDimension);
  return ProductLieGroupV43(g, h);
}

template <typename G, typename H>
ProductLieGroupV43<G, H> ProductLieGroupV43<G, H>::between(const ProductLieGroupV43& other, ChartJacobian H1,
                                                           ChartJacobian H2) const
{
  ...
  H h = traits<H>::Between(this->second, other.second, H1 ? &D_h_second : nullptr);
  if (H1)
  {
    *H1 = zeroJacobian(productDimension);
    H1->block(0, 0, firstDimension, firstDimension) = D_g_first;
...
template <typename G, typename H>
ProductLieGroupV43<G, H> ProductLieGroupV43<G, H>::inverse(ChartJacobian D) const
{
  const size_t firstDimension = firstDim();
  const size_t secondDimension = secondDim();
  const size_t productDimension = combinedDimension(firstDimension, secondDimension);
  ...
  if (D)
  {
    *D = zeroJacobian(productDimension);
    ...
    D->block(firstDimension, firstDimension, secondDimension, secondDimension) = D_h_second;
  }
  return ProductLieGroupV43(g, h);
}

template <typename G, typename H>
ProductLieGroupV43<G, H> ProductLieGroupV43<G, H>::Expmap(const TangentVector& v, ChartJacobian Hv)
{
  ...
  if constexpr (firstDynamic && secondDynamic)
  {
    ...
    throw std::invalid_argument(
        "ProductLieGroupV43::Expmap requires split tangent vectors when both "
        "factors are dynamic");
  }
  else if constexpr (firstDynamic)
  {
    ...
    {
      throw std::invalid_argument(
          "ProductLieGroupV43::Expmap tangent dimension is too small for the "
          "fixed second factor");
    }
  ...
  if (static_cast<size_t>(v.size()) != combinedDimension(firstDimension, secondDimension))
  {
    throw std::invalid_argument(
        "ProductLieGroupV43::Expmap tangent dimension does not match product "
        "dimension");
  }
  Matrix D_g_first;
  Matrix D_h_second;
  ...
  ProductLieGroupV43 result = Expmap(v1, v2,
  ...
  if (Hv)
  {
    ...
    *Hv = zeroJacobian(productDimension);
    ...
    Hv->block(0, firstDimension, productDimension, secondDimension) = D_h_second;
  }
  return result;
}

template <typename G, typename H>
ProductLieGroupV43<G, H> ProductLieGroupV43<G, H>::Expmap(const Eigen::Ref<const typename traits<G>::TangentVector>& v1,
...
template <typename G, typename H>
typename ProductLieGroupV43<G, H>::TangentVector ProductLieGroupV43<G, H>::Logmap(const ProductLieGroupV43& p,
                                                                                  ChartJacobian Hp)
{
  ...
  Jacobian1 D_g_first;
  Jacobian2 D_h_second;
  typename traits<G>::TangentVector v1 = traits<G>::Logmap(p.first, Hp ? &D_g_first : nullptr);
  typename traits<H>::TangentVector v2 = traits<H>::Logmap(p.second, Hp ? &D_h_second : nullptr);
  ...
  if (Hp)
  {
    *Hp = zeroJacobian(productDimension);
    ...
    Hp->block(firstDimension, firstDimension, secondDimension, secondDimension) = D_h_second;
  }
  return v;
}

template <typename G, typename H>
ProductLieGroupV43<G, H> ProductLieGroupV43<G, H>::expmap(const TangentVector& v) const
{
  return compose(ProductLieGroupV43::Expmap(v));
}

template <typename G, typename H>
typename ProductLieGroupV43<G, H>::Jacobian ProductLieGroupV43<G, H>::AdjointMap() const
{
...
template <typename G, typename H>
template <typename T>
T ProductLieGroupV43<G, H>::defaultIdentity()
{
...
typename traits<T>::TangentVector ProductLieGroupV43<G, H>::tangentSegment(const TangentVector& v, size_t start,
                                                                           size_t runtimeDimension)
{
  ...
  else
  {
    ...
    return v.template segment<Dimension>(startIndex);
  }
}

template <typename G, typename H>
typename ProductLieGroupV43<G, H>::TangentVector
ProductLieGroupV43<G, H>::makeTangentVector(const typename traits<G>::TangentVector& v1,
...
template <typename G, typename H>
typename ProductLieGroupV43<G, H>::Jacobian ProductLieGroupV43<G, H>::zeroJacobian(size_t productDimension)
{
  if constexpr (dimension == Eigen::Dynamic)
  {
...
template <typename G, typename H>
typename ProductLieGroupV43<G, H>::Jacobian ProductLieGroupV43<G, H>::identityJacobian(size_t productDimension)
{
  if constexpr (dimension == Eigen::Dynamic)
  {
...
template <typename G, typename H>
void ProductLieGroupV43<G, H>::checkMatchingDimensions(const ProductLieGroupV43& other, const char* operation) const
{
  if (firstDim() != other.firstDim() || secondDim() != other.secondDim())
  {
    throw std::invalid_argument(std::string("ProductLieGroupV43::") + operation +
                                " requires matching component dimensions");
  }
}

template <typename G, typename H>
void ProductLieGroupV43<G, H>::print(const std::string& s) const
{
  std::cout << s << "ProductLieGroupV43" << std::endl;
  traits<G>::Print(this->first, "  first");
  traits<H>::Print(this->second, "  second");
}

template <typename G, int N, typename Derived>
void PowerLieGroupBase<G, N, Derived>::checkDynamicTangentSize(const TangentVector& v, size_t count,
...
template <typename G, int N, typename Derived>
void PowerLieGroupBase<G, N, Derived>::checkMatchingCounts(const Derived& other, const char* operation) const
{
  if constexpr (isDynamic)
  {
    if (derived().size() != other.size())
    {
      throw std::invalid_argument(std::string("PowerLieGroup::") + operation + " requires matching component counts");
    }
  }
  else
  {
...
template <typename G, int N, typename Derived>
typename traits<G>::TangentVector PowerLieGroupBase<G, N, Derived>::tangentSegment(const TangentVector& v, size_t i)
{
  if constexpr (isDynamic)
  {
...
template <typename G, int N, typename Derived>
void PowerLieGroupBase<G, N, Derived>::assignTangentSegment(TangentVector& v, size_t i,
...
void PowerLieGroupBase<G, N, Derived>::assignJacobianBlock(MatrixType& H, size_t i, const BaseJacobian& block)
{
  if constexpr (isDynamic)
  {
  ...
  {
    H.template block<baseDimension, baseDimension>(i * baseDimension, i * baseDimension) = block;
  }
}

template <typename G, int N, typename Derived>
void PowerLieGroupBase<G, N, Derived>::fillJacobianBlocks(ChartJacobian H, const JacobianStorage& jacobians,
                                                          size_t count)
{
  if (!H)
    return;
...
template <typename G, int N, typename Derived>
Derived PowerLieGroupBase<G, N, Derived>::operator*(const Derived& other) const
{
...
template <typename G, int N, typename Derived>
Derived PowerLieGroupBase<G, N, Derived>::retract(const TangentVector& v, ChartJacobian H1, ChartJacobian H2) const
{
  ...
  for (size_t i = 0; i < count; ++i)
  {
    result[i] = traits<G>::Retract(derived()[i], tangentSegment(v, i), H1 ? &firstJacobians[i] : nullptr,
...
PowerLieGroupBase<G, N, Derived>::localCoordinates(const Derived& g, ChartJacobian H1, ChartJacobian H2) const
{
  ...
  for (size_t i = 0; i < count; ++i)
  {
    assignTangentSegment(
        v, i,
        traits<G>::Local(derived()[i], g[i], H1 ? &firstJacobians[i] : nullptr, H2 ? &secondJacobians[i] : nullptr));
  }
  fillJacobianBlocks(H1, firstJacobians, count);
  fillJacobianBlocks(H2, secondJacobians, count);
  return v;
...
template <typename G, int N, typename Derived>
Derived PowerLieGroupBase<G, N, Derived>::compose(const Derived& other, ChartJacobian H1, ChartJacobian H2) const
{
  ...
  for (size_t i = 0; i < count; ++i)
  {
    result[i] = traits<G>::Compose(derived()[i], other[i], H1 ? &jacobians[i] : nullptr);
  }
  fillJacobianBlocks(H1, jacobians, count);
  if (H2)
    *H2 = identityJacobian(count);
  return result;
}

template <typename G, int N, typename Derived>
Derived PowerLieGroupBase<G, N, Derived>::between(const Derived& other, ChartJacobian H1, ChartJacobian H2) const
{
  ...
  for (size_t i = 0; i < count; ++i)
  {
    result[i] = traits<G>::Between(derived()[i], other[i], H1 ? &jacobians[i] : nullptr);
  }
  fillJacobianBlocks(H1, jacobians, count);
  if (H2)
    *H2 = identityJacobian(count);
...
template <typename G, int N, typename Derived>
Derived PowerLieGroupBase<G, N, Derived>::Expmap(const TangentVector& v, ChartJacobian Hv)
{
  size_t count = 0;
  if constexpr (isDynamic)
  {
    ...
    count = static_cast<size_t>(v.size() / static_cast<Eigen::Index>(baseDimension));
  }
  else
  {
    count = N;
  ...
  for (size_t i = 0; i < count; ++i)
  {
    result[i] = traits<G>::Expmap(tangentSegment(v, i), Hv ? &jacobians[i] : nullptr);
  }
...
template <typename G, int N, typename Derived>
typename PowerLieGroupBase<G, N, Derived>::TangentVector PowerLieGroupBase<G, N, Derived>::Logmap(const Derived& p,
                                                                                                  ChartJacobian Hp)
{
  ...
  for (size_t i = 0; i < count; ++i)
  {
    assignTangentSegment(v, i, traits<G>::Logmap(p[i], Hp ? &jacobians[i] : nullptr));
  }
...
template <typename G, int N, typename Derived>
typename PowerLieGroupBase<G, N, Derived>::Jacobian PowerLieGroupBase<G, N, Derived>::AdjointMap() const
{
...
template <typename G, int N, typename Derived>
bool PowerLieGroupBase<G, N, Derived>::equals(const Derived& other, double tol) const
{
  if constexpr (isDynamic)
  {
    if (derived().size() != other.size())
...
template <typename G, int N, typename Derived>
typename PowerLieGroupBase<G, N, Derived>::TangentVector PowerLieGroupBase<G, N, Derived>::zeroTangent(size_t count)
{
  if constexpr (isDynamic)
  {
...
template <typename G, int N, typename Derived>
typename PowerLieGroupBase<G, N, Derived>::Jacobian PowerLieGroupBase<G, N, Derived>::zeroJacobian(size_t count)
{
  if constexpr (isDynamic)
  {
...
template <typename G, int N, typename Derived>
typename PowerLieGroupBase<G, N, Derived>::Jacobian PowerLieGroupBase<G, N, Derived>::identityJacobian(size_t count)
{
  if constexpr (isDynamic)
  {
...
PowerLieGroup<G, N>::PowerLieGroup(const std::initializer_list<G>& elements)
{
  if (elements.size() != N)
  {
    throw std::invalid_argument("PowerLieGroup: initializer list size must equal N");
  }
