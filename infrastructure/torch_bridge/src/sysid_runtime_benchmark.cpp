...
public:
  ...
  SysidBenchmark(ros::NodeHandle& nh)
      : verbose_(false), total_calls_(100), warmup_calls_(10), use_ros_service_(true), dtype_("float32"), use_cuda_(true),
        use_gpu_plant_(false), use_cuda_graph_(true), use_hybrid_mode_(false)
  {
    ...
    if (model_type.empty())
    {
      throw std::runtime_error("Could not infer model type. Expected 'model_type' in export_metadata.json or 'type' in config.json");
    }
    ...
    if (model_type == "structured")
    {
      structured_runtime_ = std::make_unique<torch_bridge::StructuredSysidRuntime>(model_path, params, poly, use_cuda_, dtype_);
      ...
      ROS_INFO("Plant params: [%.3f, %.3f, %.3f, %.3f, %.3f]", params[0], params[1], params[2], params[3], params[4]);
