...
public:
  ...
  static void copy(Observation& z, const geometry_msgs::TransformStamped& tf)
  {
    ...
    const Eigen::Quaterniond q{ Eigen::Quaterniond(tf.transformt.rotation.w, tf.transformt.rotation.x,
                                                   tf.transformt.rotation.y, tf.transformt.rotation.z) };
    z[3] = prx::quaternion_to_euler(quat)[2];
  }
...
protected:
  mushr_fg_utils_t::State _state;
  mushr_fg_utils_t::StateDot _state_dot;
  mushr_fg_utils_t::Control _ctrl;
  mushr_fg_utils_t::Ubar _ubar;
  mushr_fg_utils_t::Ubar _params_ubar_u;
