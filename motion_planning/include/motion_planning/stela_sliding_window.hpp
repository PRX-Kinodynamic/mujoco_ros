...
public:
  ...
  virtual void onInit()
  {
    ...
    _estimated_tree_publisher = private_nh.advertise<prx_models::Tree>(estimated_tree_topic, 1);
  ...
  void to_file(const bool collision = false, const bool rasied_exception = false)
  {
    ...
    while (not _factors_queue.empty())
    {
      const std::size_t id{ _factors_queue.front() };
  ...
  void update_estimated_tree()
  {
    ...
    {
      // const std::uint64_t& parent_id{ _tree.nodes[idx].parent };
      // const std::uint64_t& target_id{ _tree.nodes[idx].index };

      // if (_active_nodes.count())

      // nodes_edges.push_back(std::make_pair(parent_id, target_id));

      // DEBUG_VARS(idx, node_idx);
  ...
  void set_next_node()
  {
    ...
    else
    {
      ...
      // _active_nodes.insert(node_current.index);
  ...
  void initialize()
  {
    ...
    // _factors_queue.push(_tree.root);
  ...
  void add_tree_node()
  {
    ...
    // DEBUG_VARS(_next_tree_edge, edge.source, edge.target);
    ...
    // prx_assert(total_children == 1, "Trees not supported yet");
  ...
  {
    if (_current_past_nodes > _total_past_nodes)
    {
      const std::size_t id{ _factors_queue.front() };
      // DEBUG_VARS(id, _current_past_nodes, _total_past_nodes)
      // _values.insert(root_graph_values.second);
      _isam2_update_params.removeFactorIndices = _inserted_factors[id];
      // DEBUG_VARS(id, _x_curr);
      // DEBUG_VARS(_inserted_factors[id]);
      // const gtsam::Values estimated_values{ _isam.calculateBestEstimate() };
      // _isam.getFactorsUnsafe().printErrors(estimated_values, "Graph", SF::formatter);
  ...
  void node_info_to_file(const std::size_t node_id)
  {
    ...
    // const gtsam::Key x{ std::get<0>(keys_curr) };
    // const gtsam::Key xdot{ std::get<1>(keys_curr) };

    // auto X_cov = _isam.marginalCovariance(x);
    // auto Xdot_cov = _isam.marginalCovariance(xdot);

    // DEBUG_VARS(SF::formatter(x), SF::formatter(xdot));
    // DEBUG_VARS(X_cov);
    // DEBUG_VARS(Xdot_cov);

    // const gtsam::Key xlast{ std::get<0>(keys) };
    // const gtsam::Key xdotlast{ std::get<1>(keys) };
    // auto X_cov_last = _isam.marginalCovariance(xlast);
    // auto Xdot_cov_last = _isam.marginalCovariance(xdotlast);
    // DEBUG_VARS(SF::formatter(xlast), SF::formatter(xdotlast));
    // DEBUG_VARS(X_cov_last);
    // DEBUG_VARS(Xdot_cov_last);
  }
...
private:
  ...
  std::queue<std::size_t> _factors_queue;
