...
#include <prx_models/multivariate_gaussian.hpp>
...
public:
  ...
  using StateSampler = prx_models::lie_group_gaussian_noise_t<State>;
  using Covariance = typename StateSampler::Covariance;

  mg_reachability_t(ros::NodeHandle nh)
  {
    // PRX FILES
    ...
    const std::vector<std::shared_ptr<prx::geometry_t>> system_geoms{ _plant->geometries() };
  ...
  void collion_check()
  {
    ...
    // {
    //   query.plant_configurations = plant->configuration(state);
  ...
  bool is_safe(const ml4kp_bridge::SpacePointStamped& x_hat, const PlanMsg& plan_in, const Covariance& x0,
               const Covariance& w, const std::chrono::time_point<std::chrono::system_clock>& limit)
  {
  ...
  void trajectories_to_marker()
  {
    ...
    if (_visualize)
    {
      ...
      {
        _trajectory_markers.markers.push_back(ml4kp_bridge::create_marker(0.01, { 1, 1, 0, 0 }));
        _trajectory_markers.markers.back().type = visualization_msgs::Marker::LINE_STRIP;

        _checked_trajectories.pop_back();
        ml4kp_bridge::update_marker(_trajectory_markers.markers.back(), traj, 0, 1, 0.0);
      }
...
private:
  ...
  std::atomic<bool> _vis_thread;
  std::atomic<int> _unchecked_trajectories, _collisions_in_check;
  std::mutex _trajectories_mutex, _checked_trajectories_mutex;
  ...
  std::vector<prx::collision_checking::pqp::query_t> _queries;
