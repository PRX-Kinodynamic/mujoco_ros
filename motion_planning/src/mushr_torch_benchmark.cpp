...
public:
  ...
  SysidBenchmark(ros::NodeHandle& nh)
    ...
    , total_calls_(100)
    , warmup_calls_(10)
    , use_ros_service_(true)
    , dtype_("float32")
    , use_cuda_(true)
    , use_gpu_plant_(false)
    ...
    , use_hybrid_mode_(false)
  {
    ...
    if (model_type.empty())
    {
      throw std::runtime_error(
          "Could not infer model type. Expected 'model_type' in export_metadata.json or 'type' in config.json\n");
    }
    ...
    if (model_type == "structured")
    {
      ...
      ROS_INFO(" Plant params : [ % .3f, % .3f, % .3f, % .3f, % .3f ] ", params[0], params[1], params[2], params[3],
               params[4]);
      ROS_INFO("Steering poly: [%.3f, %.3f, %.3f, %.3f]", poly[0], poly[1], poly[2], poly[3]);
  ...
  void run_all()
  {
    ...
    std::cout << "Hybrid Mode: " << (use_hybrid_mode_ && use_cuda_ ? "enabled" : "disabled") << "\n";
  ...
  void check_jacobians()
  {
    ...
    PartialXdot partial_xdot = [&](const StateDot& xd) {
      // DEBUG_VARS(xd.transpose(), ui_.transpose());
      return structured_runtime_->call(xd, ui_);
    };
    PartialCtrl partial_ctrl = [&](const Control& u) {
      // DEBUG_VARS(xi_.transpose(), u.transpose());
      return structured_runtime_->call(xi_, u);
    };
    ...
    // std::function<gtsam::Vector(const StateDot&, const Control&)> pred_proxy =
    //     [](const StateDot& xd0, const Control& u) { return direct_runtime_->predict(xi, ui); };
    ...
    for (int i = 0; i < total_calls_; ++i)
    {
      ...
      // PRINT_MSG("****** ****** ****** ******");
      // DEBUG_VARS(xi_.transpose(), ui_.transpose());
      // DEBUG_VARS(structured_runtime_->call(xi_, ui_).transpose());
      // DEBUG_VARS(structured_runtime_->call(xi_, ui_).transpose());
      // DEBUG_VARS(structured_runtime_->call(xi_, ui_).transpose());
      // DEBUG_VARS(structured_runtime_->call(xi_, ui_).transpose());
      // auto xd1 = direct_runtime_->predict(xi_, ui_);
      ...
      auto [xd1p, actualHxd, actualHu] = structured_runtime_->predict_with_jac(xi_, ui_);
    ...
    print_stats("In-Process Jacobians torch vs Numerical", true, durations);
...
private:
  ...
  std::unique_ptr<StructuredSysidRuntime> structured_runtime_;
