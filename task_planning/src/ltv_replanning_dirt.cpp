...
{
  ros::init(argc, argv, "LTVPlanner_example");
  ros::NodeHandle n;
  ...
  n.getParam(ros::this_node::getName() + "/environment", environment);
  ...
  std::vector<double> goal_config = params["/goal/state"].as<std::vector<double>>();
  prx::space_point_t goal = ss->make_point();
  goal->at(0) = goal_config[0];
  goal->at(1) = goal_config[1];

  std_msgs::Float64 goal_radius;
  goal_radius.data = params["/goal/radius"].as<double>();
  ...
  dirt_spec->min_control_steps = params["control_steps/min"].as<int>();
  dirt_spec->max_control_steps = params["control_steps/max"].as<int>();
  dirt_spec->blossom_number = params["planner/blossom_number"].as<int>();
  dirt_spec->use_pruning = false;
  dirt_spec->use_contingency = false;

  dirt_spec->sample_state = [&](prx::space_point_t& s) {
    default_sample_state(s, dirt_spec->state_space);
  ...
  dirt_spec->cost_function = [](const prx::trajectory_t& traj, const prx::plan_t& plan) {
    double x_cost{ 0.0 };
    double u_cost{ 0.0 };
