...
struct collector_t
{
  ...
  std::vector<double> u0_bounds_min, u0_bounds_max;
  std::vector<double> control_bounds_min, control_bounds_max;
  ...
  collector_t(ros::NodeHandle nh) : collected_trajs(0)
  {
    ...
    u0_sampler.bounds(u0_bounds_min, u0_bounds_max);
  ...
  double sample_plan(const bool first, std::vector<ml4kp_bridge::PlanStepStamped>& plan)
  {
    ...
    if (first)
    {
      plan.back().plan_step.control.point = u0_sampler();
    }
    else
    {
  ...
  ml4kp_bridge::PlanTrajectory collect_trajectory()
  {
    ...
    {
      // const int random_steps{ steps_sampler() };
      const double edge_length{ sample_plan(first, plan) };
      first = false;
